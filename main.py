...
def display_image():
    ...
    d.text((text_x_pos, text_y_pos), mark_text, font=text_font, fill=(r, g, b, text_opacity))
...
btn_read_image = tk.Button(frm_options, text="Read Image", command=read_image)
btn_read_image.grid(row=1, column=0, sticky="ew", padx=2, pady=2)

ent_set_text = tk.Entry(frm_options, text="Set Text", textvariable=watermark_text)
ent_set_text.grid(row=2, column=0, sticky="ew", padx=2, pady=2, )

btn_change_color = tk.Button(frm_options, text="Change Color", command=set_watermark_text_color)
...
scl_set_X = tk.Scale(master=frm_options,
                     from_=0,
                     to=1000,
                     orient="horizontal",
                     label="X pos",
                     command=set_watermark_x_pos)
scl_set_X.grid(row=5, column=0, sticky="ew", padx=2, pady=2, )

scl_set_Y = tk.Scale(master=frm_options, from_=0, to=1000, orient="horizontal", label="Y pos",
                     command=set_watermark_y_pos)
scl_set_Y.grid(row=6, column=0, sticky="ew", padx=2, pady=2, )

scl_opacity = tk.Scale(master=frm_options, from_=0, to=255, label="Opacity", orient="horizontal",
